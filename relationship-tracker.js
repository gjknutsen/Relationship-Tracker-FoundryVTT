...
async function injectRelationshipTabV2(app, element) {
  ...
  // ApplicationV2 sheets often re-render in place (patching the existing
  // DOM) rather than tearing it down, so a section we injected earlier can
  // still be present. If so, just refresh its contents instead of
  // appending a second, duplicate section.
  const existingSection = root.querySelector('.tab.relationship-tracker-tab');
  if (existingSection) {
    existingSection.innerHTML = contentHtml;
    return;
  }
